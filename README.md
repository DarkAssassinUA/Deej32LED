...
- **Индикация статуса** — секции ленты, которые простаивают, пульсируют красным (если нет подключения к домашнему WiFi) или синим (если есть WiFi, но клиент на ПК не запущен).
...
- Настройки (сглаживание, темы, яркость) автоматически сохраняются во внутреннюю память (EEPROM) и загружаются при старте контроллера.
...
- [deejng](https://github.com/nicholasgasior/deejng) или любой другой десктопный клиент, поддерживающий OledDeej/Websockets.

### Первая загрузка прошивки (по кабелю)

```bash
# 1. Скачайте репозиторий через Git (замените юзернейм на свой, если сделали Fork)
git clone https://github.com/твое_имя_пользователя/Deej32Led.git
cd Deej32Led
